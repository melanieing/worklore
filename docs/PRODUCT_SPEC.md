...
- (2026-06-10 추가 합의) **기존 이력서 임포트** — 온보딩 및 사용 중 언제든 이력서
  파일(PDF/DOCX) 업로드 → AI가 draft 성취 카드 일괄 추출 → 사용자 검토/확정.
  콜드 스타트(빈 타임라인) 해결 + export 가치 증폭이 목적. 파일·추출 텍스트도
  원데이터로 immutable 보존(`resume_imports`), 재임포트 시 중복 카드 병합 제안.
- 대화형 기록 입력: 텍스트 + 음성(STT)
- 기록 직후 AI 역질문 1~3개 + 답변 스레드
  - (2026-06-10 합의) 스레드는 **단방향 인터뷰** 유지(사용자 자유 질문 없음).
    대신 질문별 **"힌트 요청" 1회** 제공 — 답하기 막막할 때 AI가 그 질문에 한해
    힌트/예시를 줌(소형 모델, 호출량 예측 가능). 자유 대화 코치 모드는 V2 보류.
...
- 채용공고 맞춤 테일러링 — V2. **단, V2의 핵심 수익화 기능으로 격상** (2026-06-10 합의):
  - 공고(URL/텍스트) 분석 → 요구 역량 추출 → 쌓인 성취 카드와 매칭 →
    합격 가능성 높은 맞춤 문서 생성 (이력서/경력기술서/포트폴리오/자기소개서 —
    문서 종류는 로케일별: EN은 resume/cover letter, KO는 경력기술서/자소서)
  - 과금 구조: 일반 문서 추출은 Pro 기본, **공고 맞춤 추출은 추가 과금**
    (Pro 상위 티어 또는 공고당 크레딧 — V2 시점에 A/B)
  - 차별화 근거: 경쟁 제품·범용 챗봇과 달리 우리는 지표가 신선할 때 캡처한
    정량 원데이터를 보유 → 공고 요구사항과 검증된 성취의 매칭이 가능
  - V1 의존성: 카드/불릿 데이터가 소스이므로 V1 스키마 변경 불요.
    V2에서 `job_postings`, `tailored_exports` 테이블 추가 예정
...
resume_imports    : file_url, file_name, extracted_text, status(pending|parsed|failed)
                    -- 임포트한 이력서 원본. 원데이터로 immutable 보존 (2026-06-10 추가)
achievement_cards : title, situation, task, action, result,
                    ...
                    source_import_id uuid?    -- 임포트 출처 (journal 파생이면 null)
...
| 역질문 힌트 | 소형 | 질문당 1회, 답변 예시/정량화 가이드 |
...
- 비용 목표: 사용자당 월 AI 원가 < $0.5 → **$0.10으로 하향** (2026-06-10, docs/COSTS.md)
- (2026-06-10 추가 합의) LLM은 프로바이더 중립 — 추상화 레이어 + 작업별 최저가 라우팅.
  **모델 선택을 사용자에게 노출하지 않는다.** 품질은 플랜에 묶는다:
  Free = 최저가 라우팅, Pro = 카드/불릿 상위 모델 + 카드 단위 "고품질 재생성".
  사용자 표현은 결과 기준("더 정교한 카드"), 모델명 노출 금지. 상세: docs/COSTS.md
