...
- [x] M2-1 카드 생성 프롬프트 v1 (`prompts/achievement-card.v1.ts`) — 2026-06-11
  - 입력: entry 원문 + 문답 스레드 / 출력: STAR + metrics + skills + 기간 (구조화 출력)
  - quality 라우팅 + plan 인자, 충실성 원칙(수치 날조 금지) 프롬프트에 명시
- [x] M2-2 카드 생성 서버 액션 + `source_entry_ids` 추적성 보장 — 2026-06-11
  - entry 상세에서 수동 버튼으로 draft 카드 생성 (자동 생성 안 함 — 비용 전략 §5)
- [~] M2-3 카드 편집 UI (제목/STAR 필드/metrics/skills, draft → confirmed 전환)
  - 상세 뷰 + confirm 전환 완료(`/cards/[id]`). 필드 편집 UI 남음
- [x] M2-4 타임라인 뷰 (`/timeline`) — 카드를 기간순 정렬, project_tag 필터 — 2026-06-11
  - 연도 그룹핑, 상태별 좌측 보더(green/gold), 미카드화 entry 카운트 → 저널 유도
...
- [ ] Q-7 디자인 토큰 코드화 — `docs/DESIGN.md` 팔레트를 `globals.css` @theme로 등록,
  기존 화면(M1)의 neutral-* 클래스를 토큰으로 교체 (M2 UI 작업 시작 시 함께)
