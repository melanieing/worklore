...
- LLM: **프로바이더 중립** (특정 벤더 고정 금지 — Claude API는 기본값 아님).
  Vercel AI SDK 추상화 + env 주입, 작업별 최저가 라우팅(소형 = 역질문/태깅,
  상위 = 카드/불릿). 단가 비교·라우팅 결정은 `docs/COSTS.md`가 SSOT.
- 음성 입력: 브라우저 MediaRecorder → STT API (V1은 외부 STT, 자체 구축 금지.
  기본 채택: Groq Whisper — docs/COSTS.md §3)
...
- UI 색·타이포·보이스는 `docs/DESIGN.md` (보라 잉크 팔레트)가 SSOT.
  색 하드코딩 금지 — Tailwind @theme 토큰(`bg-iris`, `text-ink` 등) 사용.
  보라 면적 10% 이하, 그라데이션 금지, AI 요소는 lavender+iris 시그니처.
...
전체 작업 단위 백로그는 `docs/BACKLOG.md` (작업 ID, 상태, 완료 기준). 마일스톤 요약:
...
- [x] M2-0 (2026-06-10): AI 레이어를 Vercel AI SDK로 이관 — **@anthropic-ai/sdk 제거됨**.
  - `src/lib/ai/provider.ts`: 모델 spec `"provider:model-id"` 해석 (openai/google/anthropic)
  - `src/lib/ai/models.ts`: `getModel(task, plan)` — light/quality × free/pro 라우팅, env 오버라이드
  - 구조화 출력은 `generateObject` + zod schema. 새 AI 작업은 반드시 이 레이어를 통할 것.
- [x] M2-1/M2-2 (2026-06-11): 카드 생성 — `src/lib/ai/achievement-card.ts`
  (quality+plan 라우팅, zod 스키마), 프롬프트 `prompts/achievement-card.v1.ts`,
  액션 `src/app/(app)/cards/actions.ts` (generateCardFromEntry/confirmCard),
  상세 페이지 `/cards/[id]`. entry → 카드 추적은 `source_entry_ids` 배열
  (`.contains()` 쿼리). 카드는 수동 버튼으로만 생성(자동 금지 — 비용 전략).
- [x] M2-4 (2026-06-11): 타임라인 `/timeline` — 연도 그룹핑, ?tag= 필터,
  미카드화 entry 카운트(전체 entry − source_entry_ids 합집합). 헤더 네비 추가.
- 남은 M2: M2-3 필드 편집 UI, M2-5 재생성, M2-6 병합, M2-7 태깅, M2-8 힌트.
